...
> 本文档供新会话接手时快速了解项目全貌。最后更新：v5.88.0（2026-09-19）。
...
**当前版本**：v5.88.0（git 152 次提交，从 v0.1 到 v5.88.0）
...
- 语录核实组 53 张卡（vs 账本引文块 56 个，白名单豁免 3 项）
...
| 脚本 | 作用 | 何时运行 |
|---|---|---|
| `verify.py` | **一键体检**：断链/重复id/版本一致/索引一致/时间轴一致/修订历史一致/语录卡覆盖（白名单核对） | **每轮必跑** |
...
3. **reading.html 长卷更新**——第肆/伍/陆/柒/捌/玖/拾章仍是跳转卡（可逐步填充为连续叙事）
4. **search.html 公司过滤纳入新页**——controversy/chronicle/finance 目前不在索引中
5. **排版精进**——深读页表格窄屏横滑已做，可继续优化

## 十、已知技术债务

1. **search.html 匹配文本**：现在含 bg 字段（背景段），但仅限 primary 类型；其他类型（文档/访谈/帖史）的 bg 字段已加入索引但检索匹配未纳入
2. **build-search-index.py 断言**：每轮新增条目后需手动更新数量断言
3. **sync-changelog.py**：条目标题含版本号+后缀（如"v5.85.0 轮"）时可能解析异常（cosmetic，不影响功能）
4. **打印缓存**：IAB 内嵌浏览器对 style.css 有顽固缓存——QA 用「服务器端 curl 确认 + 页内注入等价规则实测几何」证据链方法
