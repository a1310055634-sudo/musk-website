// 由 tools/build-search-index.py 自动生成，请勿手改。
window.SEARCH_INDEX = [
 {
  "id": "e2002-10-03",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2002.10.03",
  "s": "PayPal 交割 · 本人 2012–2013 访谈自述（USA Today 等）",
  "q": "“My proceeds from PayPal after tax were about $180M. $100M of that went into SpaceX, $70M into Tesla, and $10M into SolarCity. I had to borrow money for rent.”",
  "zh": "我 PayPal 的税后所得大约 1.8 亿美元。其中 1 亿投进了 SpaceX，7000 万投进 Tesla，1000 万投进 SolarCity。然后我不得不借钱付房租。",
  "bg": "On October 3, 2002, eBay closed its $1.5 billion acquisition of PayPal. Musk, 31, walked away with roughly $180 million after tax — enough to never work again. The next decision was where the money would go, and it is the founding decision of everything else on this site.",
  "c": [
   "PayPal",
   "SolarCity",
   "Tesla",
   "SpaceX"
  ]
 },
 {
  "id": "e2006-08",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2006.08",
  "s": "Tesla 官方博客 · The Secret Tesla Motors Master Plan",
  "q": "“Build sports car. Use that money to build an affordable car. Use that money to build an even more affordable car. While doing above, also provide zero-emission electric power generation options. Don't tell anyone.”",
  "zh": "造一辆跑车；用赚到的钱造一辆更便宜的车；再用赚到的钱造更便宜的；与此同时，提供零排放的发电选项。别告诉任何人。",
  "bg": "Tesla in 2006 was a three-year-old startup making a six-figure roadster for the rich, and the industry consensus was that electric cars would never scale. Musk, then chairman, wrote the company's real strategy on the official blog — and disguised it as a joke.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "d2006-08",
  "pg": "documents.html",
  "t": "一手文档",
  "d": "2006.08",
  "s": "The Secret Tesla Motors Master Plan（just between you and me）",
  "q": "“As you know, the initial product of Tesla Motors is the Roadster…”",
  "zh": "",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "i2006-08",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2006.08",
  "s": "把公司战略写成一句玩笑",
  "q": "“Build sports car. Use that money to build an affordable car. Use that money to build an even more affordable car… Don't tell anyone.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2008-09-28",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2008.09.28",
  "s": "Space.com · Spaceflight Now · 现场约 500 名员工",
  "q": "“As the saying goes, the fourth time's the charm… This is one of the best days of my life.”",
  "zh": "常言道，第四次总会有好运……这是我人生中最好的日子之一。",
  "bg": "SpaceX was six years old with three rockets blown up, and Musk had put in roughly the last of his PayPal money — by his own repeated public account, he was borrowing for rent. Wall Street was in the worst week of the financial crisis. A fourth failure would most likely end the company.",
  "c": [
   "PayPal",
   "SpaceX"
  ]
 },
 {
  "id": "i2008-09-28",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2008.09.28",
  "s": "「第四次总会有好运」",
  "q": "“As the saying goes, the fourth time's the charm… This is one of the best days of my life.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2008-12-24",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2008.12.24",
  "s": "Business Insider · 巴黎演讲（2015.12）· 本人 X 自述",
  "q": "“We actually closed the financing round on Christmas Eve 2008. It was the last hour of the last day that it was possible.”",
  "zh": "我们其实是在 2008 年圣诞夜关闭那一轮融资的。那是可能的最后一天的最后一个小时。",
  "bg": "December 2008: the financial crisis was at full depth, SpaceX had just been saved by NASA's $1.6 billion cargo contract on December 23, and Tesla was next in line to die — burning cash on Model S development with the round collapsing under the credit crunch. Musk had already put in his own last money and, by his own account, was borrowing for rent.",
  "c": [
   "Tesla",
   "SpaceX"
  ]
 },
 {
  "id": "e2010-06-29",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2010.06.29",
  "s": "Tesla IPO · 纳斯达克 · 公开报道口径",
  "q": "2008 年圣诞夜的救援买来两年时间；到 2010 年年中，Tesla 需要资金建 Model S 产线。半个世纪以来汽车公司在公开市场一直是毒资产——Tesla 之前，上一家上市的美国车企是 1956 年的福特。",
  "zh": "",
  "bg": "The Christmas Eve rescue of 2008 bought two years; by mid-2010 Tesla needed capital to build the Model S plant. Automakers had been toxic on public markets for half a century — the last American carmaker to IPO before Tesla was Ford in 1956.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "e2012-06-22",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2012.06.22",
  "s": "Fremont 工厂首批交付 · Forbes 现场报道 · Tesla IR",
  "q": "“It’s about breaking a spell. The world has been under this illusion that electric cars cannot be as good as gasoline cars. It’s showing that an electric car can be better than a gasoline car — it can be the best car, period.”",
  "zh": "这关乎打破一个魔咒。全世界一直活在一种幻觉里，以为电动车不可能比燃油车好。而它要证明的是电动车可以比燃油车更好——可以是最好的车，句号。",
  "bg": "Three and a half years after the Christmas Eve financing, Tesla had to prove the bet could make a real car. Roadster had been a hand-built experiment on a Lotus chassis; Model S was the first car designed from the ground up as an electric sedan, built in the former NUMMI plant in Fremont. The company was still small, the auto industry was watching politely, and the first invitations went out.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "e2013",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2013",
  "s": "访谈辑录（约 2013，广泛征引）",
  "q": "“I would like to die on Mars. Just not on impact.”",
  "zh": "我想死在火星上。但别是摔死的。",
  "bg": "SpaceX was eleven years old, still burning cash, still without a crewed mission. Asked — again — why he kept pouring money into rockets instead of enjoying his fortune, he compressed the whole Mars argument into two sentences.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "e2013-08-12",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2013.08.12",
  "s": "Hyperloop Alpha 白皮书 · Tesla 官网 57 页 PDF",
  "q": "“How could it be that the home of Silicon Valley and JPL – doing incredible things like indexing all the world’s knowledge and putting rovers on Mars – would build a bullet train that is both one of the most expensive per mile and one of the slowest?”",
  "zh": "硅谷和 JPL 的老家——做着给全世界知识建索引、往火星放漫游车这种奇事的地方——怎么会修一条每英里最贵、速度又最慢的高铁之一？",
  "bg": "California had just approved a high-speed rail project that would take years and tens of billions of dollars to connect LA and San Francisco. Musk, who had to split his time between rockets, electric cars and a solar company, did not have the bandwidth to fix it himself — so he did what a systems engineer does when overloaded: he wrote the spec and gave it away.",
  "c": [
   "Tesla",
   "Boring Company"
  ]
 },
 {
  "id": "e2014-05-29",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2014.05.29",
  "s": "Dragon V2 发布会 · Universe Today · NBC · New Scientist",
  "q": "“It can land anywhere on Earth with the accuracy of a helicopter … I think that's what a spaceship should be able to do.”",
  "zh": "它可以像直升机一样精准地降落在地球上的任何地方……我认为一艘飞船就应该能做到这一点。",
  "bg": "NASA was about to pick who would fly its astronauts, and SpaceX's cargo Dragon was a freight truck by design. On May 29, 2014, at SpaceX headquarters in Hawthorne, the crewed Dragon V2 was unveiled: seven seats, touchscreen interiors, and SuperDraco thrusters built to serve as both launch-escape engines and landing engines.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "e2014-06-12",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2014.06.12",
  "s": "TESLA 官方博客 · All Our Patent Are Belong To You",
  "q": "“Tesla will not initiate patent lawsuits against anyone who, in good faith, wants to use our technology.”",
  "zh": "Tesla 不会对任何善意使用我们技术的人发起专利诉讼。",
  "bg": "In 2014 the electric car market was a rounding error, incumbents treated EVs as compliance products, and patents were the sacred moat of any technology company. Tesla held patents on its battery pack, powertrain and Supercharger technology — and Musk decided the real enemy was not competition but the size of the market itself.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "e2015-04-30",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2015.04.30",
  "s": "Tesla Energy 发布会（TechCrunch · ABC News 记录）",
  "q": "“The issue with existing batteries is that they suck. They are expensive, unreliable and bad in every way.” — “We're talking about trying to change the fundamental energy infrastructure of the world.”",
  "zh": "现有电池的问题在于它们很烂：贵、不可靠、一无是处。——我们在尝试改变的，是全世界的能源基础设施。",
  "bg": "Before the keynote, the press expected a car. Musk walked out with a wall-mounted battery instead — the Powerwall, plus a utility-scale sibling called Powerpack, together branded “Tesla Energy.”",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "i2015-04-30",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2015.04.30",
  "s": "「现有电池的问题在于它们很烂」",
  "q": "“The issue with existing batteries is that they suck. They are expensive, unreliable and bad in every way.” — “We're talking about trying to change the fundamental energy infrastructure of the world.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2015-12-21",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2015.12.21",
  "s": "降落夜电信会议（NPR · Guardian · 会议记录存档）",
  "q": "“This is a fundamental step change in technology compared to any other rocket that has ever flown.”",
  "zh": "与史上飞过的任何其他火箭相比，这是技术上的根本性台阶变化。",
  "bg": "Every orbital rocket before this day was a disposable: flown once, then dropped into the ocean. Reusability was SpaceX's founding bet — the whole point of the company was to cut the cost of reaching space — but by December 2015 it had never been demonstrated. The return-to-flight mission after the June CRS-7 loss carried 11 Orbcomm satellites, and a first-stage return to Landing Zone 1 was a secondary objective most observers still treated as a lottery ticket.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "e2016",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2016",
  "s": "对投资者的表述（Teslarati · Ars Technica 报道）",
  "q": "“The machine that builds the machine.”",
  "zh": "造机器的机器。",
  "bg": "With Model X stumbling and Model 3 ramping up next, Musk told investors that the factory itself — not any single car — was Tesla's most important product. The fully automated future factory even had an internal codename: “alien dreadnought.”",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "i2016",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2016",
  "s": "「造机器的机器」",
  "q": "“The machine that builds the machine.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2016-03-31",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2016.03.31",
  "s": "Model 3 首批预订发布活动",
  "q": "“You won't be able to buy a car this good for this price ever again.”",
  "zh": "这个价位，你再也买不到这么好的车了。",
  "bg": "Tesla had proven it could make premium cars. The next step — the mass-market Model 3 at $35,000 — was the promise the entire company was built on. Musk walked onto the stage at the SpaceX design studio in Hawthorne and unveiled the car to a live audience and a live webcast.",
  "c": [
   "Tesla",
   "SpaceX"
  ]
 },
 {
  "id": "e2016-07-20",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2016.07.20",
  "s": "Tesla 官方博客（SEC 备案）",
  "q": "“Create stunning solar roofs… Expand the electric vehicle product line to address all major segments… Develop a self-driving capability that is 10x safer than manual… Enable your car to make money for you.”",
  "zh": "打造惊艳的太阳能屋顶并无缝整合电池储能；把电动车产品线扩展到所有主要细分市场；开发比人工驾驶安全十倍的自动驾驶；让你的车在你不用时替你赚钱。",
  "bg": "The first master plan was in its final stages of completion. Energy was the missing half: SolarCity, run by his cousins and chaired by him, was the biggest residential solar installer in the US — and Tesla had just started shipping batteries. Merging them was controversial long before it happened.",
  "c": [
   "SolarCity",
   "Tesla"
  ]
 },
 {
  "id": "d2016-07-20",
  "pg": "documents.html",
  "t": "一手文档",
  "d": "2016.07.20",
  "s": "Master Plan, Part Deux",
  "q": "“Create stunning solar roofs and seamlessly integrated battery storage.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2016-09-01",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2016.09.01",
  "s": "LC-40 爆燃事故 · 本人推文（2016.09.09）· SpaceNews",
  "q": "“Still working on the Falcon fireball investigation. Turning out to be the most difficult and complex failure we have ever had in 14 years.”",
  "zh": "仍在跟进猎鹰火球的调查。结果证明这是我们 14 年来遇到过的最困难、最复杂的失败。",
  "bg": "Falcon 9 had become the workhorse, and a routine fueling for a static fire two days before launch was supposed to be uneventful. Instead, on September 1, 2016, the rocket and its payload — the Amos-6 satellite Facebook was counting on for its internet project — burned on Pad 40 in broad daylight. No one was hurt; the rocket, the satellite and the launch schedule were not so lucky.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "e2016-09-27",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2016.09.27",
  "s": "第 67 届国际宇航大会（IAC）· 瓜达拉哈拉",
  "q": "“Making Humans a Multi-Planetary Species.”",
  "zh": "让人类成为多行星物种。",
  "bg": "Falcon 9 had just landed for the Nth time, and the Mars talk needed engineering behind it. At the 67th International Astronautical Congress in Guadalajara, he unveiled the Interplanetary Transport System (ITS) — the first full architecture for settling Mars.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "i2016-09-27",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2016.09.27 · 瓜达拉哈拉",
  "s": "「让人类成为多行星物种」",
  "q": "“Making Humans a Multi-Planetary Species.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2016-11",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2016.11",
  "s": "Tesla 股东投票 · SEC 备案",
  "q": "26 亿美元收购 SolarCity——一家由他表兄弟创办、他本人任董事长和最大股东的公司——是他职业生涯中最具争议的交易。股东在 11 月投了赞成票，但诉讼持续了数年。",
  "zh": "",
  "bg": "The $2.6B acquisition of SolarCity — a company founded by his cousins, where he was chairman and largest shareholder — was the most controversial deal of his career. Shareholders approved it in November, but the lawsuit lasted for years.",
  "c": [
   "SolarCity",
   "Tesla"
  ]
 },
 {
  "id": "e2017-03-30",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2017.03.30",
  "s": "SES-10 发布会 · Universe Today · SpaceNews",
  "q": "“This represents the culmination of 15 years of work at SpaceX to be able to refly a rocket booster. It’s really a great day, I think, for space as a whole.”",
  "zh": "这代表着 SpaceX 十五年工作的集大成——让一枚火箭助推器可以再次飞行。我想，对整个航天事业来说，这真是了不起的一天。",
  "bg": "The 2015 landing proved a booster could come back; a demo, however, is not a business. That changed when a paying customer — satellite operator SES — agreed to fly its payload on the booster that had already flown CRS-8 a year earlier. The customer even named the deal after the accident-torn history: this was the flight where reuse stopped being a stunt.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "e2017-07-28",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2017.07.28",
  "s": "Model 3 交付活动（Isaacson 传记引述）",
  "q": "“Welcome to production hell! That's where we're going to be for at least six months.”",
  "zh": "欢迎来到生产地狱！至少未来六个月，我们就待在这儿。",
  "bg": "Model 3 reservations had piled up at an unprecedented scale, and every one of them was a promise Tesla now had to keep. The company had never built a car at volume; the target was 5,000 a week, and the cash clock was ticking.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "i2017-07-28",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2017.07.28",
  "s": "「欢迎来到生产地狱」",
  "q": "“Welcome to production hell! That's where we're going to be for at least six months.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2018-01-28",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2018.01.28",
  "s": "X 帖（原帖 status/957441956874919936 · The Guardian 报道）",
  "q": "“When the zombie apocalypse happens, you'll be glad you bought a flamethrower. Works against hordes of the undead or your money back!”",
  "zh": "丧尸末日来临时，你会庆幸自己买了火焰喷射器。对不死军团有效，无效退款！",
  "bg": "To fund the first Boring Company test tunnel in Los Angeles, his team sold “Not-a-Flamethrower” units at $500 each — merchandise as financing. The internet was unsure whether it was a joke; he made it a sales campaign.",
  "c": [
   "Boring Company"
  ]
 },
 {
  "id": "p2018-01-28",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2018.01.28",
  "s": "@elonmusk",
  "q": "When the zombie apocalypse happens, you'll be glad you bought a flamethrower. Works against hordes of the undead or your money back!",
  "zh": "丧尸末日来临时，你会庆幸自己买了火焰喷射器。对不死军团有效，无效退款！",
  "c": [
   "X / Twitter",
   "Boring Company"
  ]
 },
 {
  "id": "e2018-02-06",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2018.02.06",
  "s": "猎鹰重型首飞发布会 · Space.com · AP 通稿",
  "q": "“I think it looks so ridiculous and impossible. And you can tell it’s real because it looks so fake, honestly. We’d have way better CGI if it was fake.”",
  "zh": "我觉得它看起来太荒诞、太不可能了。你之所以能看出它是真的，恰恰是因为它看起来那么假——说实话，要真是假的，我们的特效早该做得好得多。",
  "bg": "Falcon Heavy — three Falcon-9 cores strapped together, 27 engines — had been mocked since its 2011 announcement as a PowerPoint rocket. Real customers refused to risk payloads on a maiden flight, so the demo needed something heavy and useless enough to sacrifice. Musk put up his own midnight-cherry Roadster, with a mannequin in a spacesuit, to a Mars-transfer orbit.",
  "c": [
   "Tesla",
   "SpaceX"
  ]
 },
 {
  "id": "i2018-04",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2018.04",
  "s": "「人类被低估了」——给自己的公开纠错",
  "q": "“Excessive automation at Tesla was a mistake. To be precise, my mistake. Humans are underrated.”",
  "zh": "",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "e2018-08-07",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2018.08.07",
  "s": "X（原 Twitter）帖 · SEC 诉讼与和解文件",
  "q": "“Am considering taking Tesla private at $420. Funding secured.”",
  "zh": "正考虑以每股 420 美元将 Tesla 私有化。资金已到位。",
  "bg": "Model 3 had just clawed out of production hell, short sellers were circling Tesla's stock, and Musk was openly frustrated with Wall Street's short-termism. On the morning of August 7, 2018, he typed out one sentence that turned a stock into a courtroom.",
  "c": [
   "Tesla",
   "X / Twitter"
  ]
 },
 {
  "id": "d2018-08-07",
  "pg": "documents.html",
  "t": "一手文档",
  "d": "2018.08.07",
  "s": "Taking Tesla Private（致员工私有化方案信）",
  "q": "“As a public company, we are subject to wild swings in our stock price that can be a major distraction for everyone working at Tesla. Being public also subjects us to the quarterly earnings cycle that puts enormous pressure on Tesla to make decisions that may be right for a given quarter, but not necessarily right for the long-term.”",
  "zh": "",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "p2018-08-07",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2018.08.07",
  "s": "@elonmusk",
  "q": "Am considering taking Tesla private at $420. Funding secured.",
  "zh": "正考虑以每股 420 美元将 Tesla 私有化。资金已到位。",
  "c": [
   "Tesla",
   "X / Twitter"
  ]
 },
 {
  "id": "e2018-12-18",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2018.12.18",
  "s": "Boring Company 首条隧道通车仪式",
  "q": "Boring Company 始于一条关于堵车的玩笑推文。两年后，霍桑的第一条测试隧道通车——一辆改装 Model X 在自动驾驶引导轮上穿行而过。",
  "zh": "",
  "bg": "The Boring Company started as a joke tweet about traffic. Two years later, the first test tunnel in Hawthorne opened — with a modified Model X driving through it on autopilot-guided wheels.",
  "c": [
   "Tesla",
   "Boring Company"
  ]
 },
 {
  "id": "e2019-09-28",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2019.09.28",
  "s": "Starship Mk1 发布会 · Ars Technica · Popular Mechanics",
  "q": "“Honestly, I'm in love with steel.”",
  "zh": "说实话，我爱上了钢。",
  "bg": "The big Falcon Rocket had been mocked since 2016 for a carbon-fiber plan that priced itself out of existence. In 2019 Musk switched the whole design to stainless steel — cheaper, heat-tolerant, and buildable in open air fields in Boca Chica. On the eleventh anniversary of Falcon 1's orbit, he showed the Mk1 prototype.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "e2019-11-21",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2019.11.21",
  "s": "Cybertruck 发布会 · X 帖解释（BBC 报道）",
  "q": "“Sledgehammer impact on door cracked the base of the glass, which is why the steel ball didn't bounce off. Should have done steel [balls] on wheels.”",
  "zh": "大锤砸门时已经震裂了玻璃底部，所以钢球没有弹开。应该先把钢球砸车轮。",
  "bg": "Tesla's pickup reveal in Hawthorne was staged as an anti-Keynote: brutalist design, a steel-ball durability demo, zero polish. The pitch depended on one claim — the “armor glass” was unbreakable.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "p2019-11-21",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2019.11.21",
  "s": "@elonmusk",
  "q": "Sledgehammer impact on door cracked the base of the glass, which is why the steel ball didn't bounce off. Should have done steel [balls] on wheels.",
  "zh": "大锤砸门时已震裂玻璃底部，所以钢球没有弹开。应该先把钢球砸车轮。",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "i2019-11-22",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2019.11.22",
  "s": "「大锤砸门」的技术解释",
  "q": "“Sledgehammer impact on door cracked the base of the glass, which is why the steel ball didn't bounce off. Should have done steel [balls] on wheels.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2020-05-30",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2020.05.30",
  "s": "NASA 官方通稿 · NASA TV 直播（Business Insider 记录）",
  "q": "“This is a dream come true for me and everyone at SpaceX. It is the culmination of an incredible amount of work by SpaceX and NASA teams to realize this goal.” — “This day heralds a new age of space exploration… I'm not very religious, but I prayed for this one.”",
  "zh": "对我和 SpaceX 的所有人来说，这是梦想成真。它是 SpaceX 与 NASA 两支团队无数工作的集大成。——这一天宣告了太空探索新纪元的到来……我不太信教，但这一次我祈祷了。",
  "bg": "Eighteen years after founding SpaceX with the goal of making life multiplanetary, and after Shuttle retirement left America without a crewed launcher, two NASA astronauts strapped into a SpaceX capsule. The company he built to reach Mars had just become the transport service for NASA.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "i2020-05-30",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2020.05.30",
  "s": "「我不太信教，但这一次我祈祷了」",
  "q": "“This is a dream come true for me and everyone at SpaceX.” — “I'm not very religious, but I prayed for this one.”",
  "zh": "",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "e2020-09-22",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2020.09.22",
  "s": "Battery Day 发布会（Rev.com 全文转录 · Ars Technica 报道）",
  "q": "“…when we put it all together and go to our new 80 millimeter length, 4680…” — the $25,000 car was the promise the whole deck was built around.",
  "zh": "……当我们把这些全部组合起来、切换到新的 80 毫米长度——4680——的时候……（全场演示都围绕着「2.5 万美元的车」这一承诺展开。）",
  "bg": "Tesla was at peak profitability, and Musk picked this moment to attack the most expensive part of the car: the battery. The keynote promised a tabless 4680 cell, in-house production, and — the headline — a path to a $25,000 electric car.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "p2021-03-02",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2021.03.02",
  "s": "@elonmusk",
  "q": "The Boca Chica Village area (part of Cameron County) will be incorporated as Starbase, Texas.",
  "zh": "博卡奇卡村（卡梅伦县的一部分）将并入为德克萨斯州 Starbase。",
  "c": [
   "SpaceX",
   "X / Twitter"
  ]
 },
 {
  "id": "e2021-07",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2021.07",
  "s": "特拉华衡平法院庭审作证（The Next Web 报道）",
  "q": "He said he “hates” being Tesla's CEO — but kept the job because, without him, the company would “die”.",
  "zh": "他说自己「讨厌」当 Tesla 的 CEO——但留着这份工作，因为没有他，公司会「死」。",
  "bg": "Five years after the $2.6B SolarCity acquisition, shareholders sued him and Tesla's board over the related-party deal. In July 2021 he took the witness stand in Wilmington, Delaware — under oath, for hours.",
  "c": [
   "SolarCity",
   "Tesla"
  ]
 },
 {
  "id": "i2021-07",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2021.07",
  "s": "「讨厌当 CEO，但没有我公司会死」",
  "q": "He said he “hates” being Tesla's CEO — but kept the job because, without him, the company would “die”.",
  "zh": "",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "e2021-08",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2021.08",
  "s": "Tesla AI Day（CNBC · Reuters 报道，2022.09.30 复述）",
  "q": "“The Optimus robot will eventually be worth more than the car business, worth more than FSD.”",
  "zh": "Optimus 机器人最终的价值将超过汽车业务，超过 FSD。",
  "bg": "Tesla was at peak profitability with two robot programs quietly running. At the first AI Day, Musk put one slide up that wasn't a vehicle at all: a person-shaped robot named Optimus.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "e2022-04-14",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2022.04.14",
  "s": "TED 大会访谈（TechCrunch · Washington Post 记录）",
  "q": "“I don't care about the economics at all… This is not a way to make money… an inclusive arena for free speech… important to the future of civilization.”",
  "zh": "我根本不在乎经济账……这不是一门用来赚钱的生意……一个包容的言论自由广场……关乎文明的未来。",
  "bg": "He had quietly become Twitter's largest single shareholder, declined a board seat, and decided to bid for the whole company. The TED stage in Vancouver was where he explained — live — why a carmaker's CEO wanted a social network.",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "i2022-04-14",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2022.04.14",
  "s": "「我根本不在乎经济账」",
  "q": "“I don't care about the economics at all… This is not a way to make money… an inclusive arena for free speech… important to the future of civilization.” — “I'm not actually sure I will be able to acquire it.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "d2022-04-25",
  "pg": "documents.html",
  "t": "一手文档",
  "d": "2022.04.25",
  "s": "Agreement and Plan of Merger（收购协议关键条款摘录）",
  "q": "“At the effective time, each Share issued and outstanding immediately prior to the Effective Time … shall be converted into the right to receive $54.20 in Cash, without interest …” —— Section 2.1(a) Conversion; Exchange of Shares",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2022-10-26",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2022.10.26",
  "s": "X 帖（原帖 status/1585341984679469056）",
  "q": "“Entering Twitter HQ – let that sink in!”",
  "zh": "正在进入推特总部——让「水槽」（这件事）沉进去吧。",
  "bg": "Two days before the court-ordered deadline to close the $44B Twitter deal, with the fight behind him and the paperwork nearly done, Musk walked into Twitter's San Francisco headquarters carrying a bathroom sink.",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "p2022-10-26",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2022.10.26",
  "s": "@elonmusk",
  "q": "Entering Twitter HQ – let that sink in!",
  "zh": "正在进入推特总部——让「水槽」（这件事）沉进去吧。",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "e2022-10-28",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2022.10.28",
  "s": "X 帖 · Reuters · Washington Post",
  "q": "“The bird is freed.” — “Spoiler alert: Let the good times roll.”",
  "zh": "鸟儿自由了。——剧透警告：让好日子滚滚而来。",
  "bg": "He had tried to walk away, been sued, and been forced to close at the original $54.20 a share. On October 28, 2022, the $44B deal was done and he owned the world's town square.",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "p2022-10-28",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2022.10.28",
  "s": "@elonmusk",
  "q": "The bird is freed. Spoiler alert: Let the good times roll",
  "zh": "鸟儿自由了。——剧透警告：让好日子滚滚而来。",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "e2022-11-16",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2022.11.16",
  "s": "深夜全员邮件（Fortune · The Verge 披露全文）",
  "q": "“Going forward, to build a breakthrough Twitter 2.0… this will mean working long hours at high intensity. Only exceptional performance will constitute a passing grade.”",
  "zh": "往后，要打造一个突破性的 Twitter 2.0……这意味着长时间、高强度的工作。唯有卓越的表现才算及格。",
  "bg": "Three weeks into ownership: mass layoffs already done, advertisers pausing over content-moderation worries, and a company burning cash. Musk chose the night to send the starkest email of his career.",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "d2022-11-16",
  "pg": "documents.html",
  "t": "一手文档",
  "d": "2022.11.16 深夜",
  "s": "Extremely Hardcore（全员邮件）",
  "q": "“Going forward, to build a breakthrough Twitter 2.0 and succeed in an increasingly competitive world, we will need to be extremely hardcore.”",
  "zh": "",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "i2022-11-16",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2022.11.16",
  "s": "「唯有卓越的表现才算及格」",
  "q": "“Going forward, to build a breakthrough Twitter 2.0… we will need to be extremely hardcore… Only exceptional performance will constitute a passing grade.”",
  "zh": "",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "e2022-11-28",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2022.11.28",
  "s": "X 帖预告 · Fox Business 报道（Twitter Files 系列首发于 2022.12.02）",
  "q": "“The public deserves to know what really happened and how it happened.” （广泛征引措辞 · 逐字文本未经原帖存档复核，特此注明）",
  "zh": "公众理应知道究竟发生了什么、是如何发生的。（此句为广泛征引措辞，本站无法核实原帖逐字文本，特此注明。）",
  "bg": "Five weeks into owning Twitter, he handed internal documents to journalists Matt Taibbi and Bari Weiss — the start of the “Twitter Files” series. He teased the release the day before Thanksgiving week.",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "p2022-11-28",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2022.11.28",
  "s": "@elonmusk",
  "q": "The public deserves to know what really happened and how it happened.",
  "zh": "公众理应知道究竟发生了什么、是如何发生的。",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "e2023-03-01",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2023.03.01",
  "s": "Tesla Investor Day · 官方文档",
  "q": "“Sustainable Energy for All of Earth.”",
  "zh": "让全地球都用上可持续能源。",
  "bg": "By 2023 the questions had shifted from “can Tesla survive” to “can the planet electrify”. At Investor Day in Austin (March 1) he teased the third master plan; the full document landed on April 5.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "d2023-04-05",
  "pg": "documents.html",
  "t": "一手文档",
  "d": "2023.03.01 Investor Day 发布 · 2023.04.05 全文",
  "s": "Master Plan Part 3 — Sustainable Energy for All of Earth",
  "q": "“Sustainable Energy for All of Earth.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2023-07-12",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2023.07.12",
  "s": "xAI 官宣 · Twitter Spaces（Reuters 报道 07.14）",
  "q": "“To understand the true nature of the universe.” — “If it tried to understand the true nature of the universe, that's actually the best thing that I can come up with from an AI safety standpoint.”",
  "zh": "理解宇宙的真实本质。——「如果它试图理解宇宙的真实本质，从 AI 安全的角度看，这是我能想到的最好方案。」",
  "bg": "OpenAI — which he co-founded and had left — had gone closed-source and commercial; he openly disagreed with its direction. His answer was to start a rival with a deliberately simple charter.",
  "c": [
   "X / Twitter",
   "xAI"
  ]
 },
 {
  "id": "d2023-07-12",
  "pg": "documents.html",
  "t": "一手文档",
  "d": "2023.07.12 / 07.14",
  "s": "xAI 宣言（章程级一句话）",
  "q": "“To understand the true nature of the universe.”",
  "zh": "",
  "c": [
   "xAI"
  ]
 },
 {
  "id": "i2023-07-14",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2023.07.14",
  "s": "「从 AI 安全的角度，这是我能想到的最好方案」",
  "q": "“If it tried to understand the true nature of the universe, that's actually the best thing that I can come up with from an AI safety standpoint.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "p2023-11-04",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2023.11.04",
  "s": "@elonmusk",
  "q": "As soon as it's out of early beta, xAI's Grok system will be available to all X Premium+ subscribers.",
  "zh": "一旦结束早期测试，xAI 的 Grok 系统将面向所有 X Premium+ 订阅用户开放。",
  "c": [
   "X / Twitter",
   "xAI"
  ]
 },
 {
  "id": "e2023-11-29",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2023.11.29",
  "s": "NYT DealBook 峰会访谈（CNBC · Wired 记录）",
  "q": "“If somebody's going to try to blackmail me with advertising, they can go f*** themselves… What this advertising boycott is going to do is it's going to kill the company… and the whole world will know that those advertisers killed the company.”",
  "zh": "如果有人想用广告来要挟我，那就滚自己的吧……这场广告抵制会做什么？它会杀死这家公司……全世界都会知道，是那些广告主杀死了它。",
  "bg": "Weeks after his own post drew advertiser walkouts, X's revenue was bleeding and Disney's CEO had joined the boycott. On the DealBook stage, Sorkin asked whether he had a message for the advertisers leaving.",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "i2023-11-29",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2023.11.29",
  "s": "「让全世界知道是谁杀死了这家公司」",
  "q": "“If somebody's going to try to blackmail me with advertising, they can go f*** themselves… The whole world will know that those advertisers killed the company.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "p2023-11-29",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2023.11.29",
  "s": "@elonmusk",
  "q": "（非帖文）DealBook 峰会访谈实录：「If somebody's going to try to blackmail me with advertising… The whole world will know that those advertisers killed the company.」",
  "zh": "（访谈而非帖文）如果有人想用广告来要挟我……全世界都会知道，是那些广告主杀死了这家公司。",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "e2023-11-30",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2023.11.30",
  "s": "Cybertruck 首批交付活动（CNBC · Fortune · Business Insider 记录）",
  "q": "“Finally, the future will look like the future.” — “We have a car here that experts said was impossible, that experts said would never be [made].”",
  "zh": "终于，未来看起来像未来了。——这辆车，专家们曾说不可能，说永远不会被造出来。",
  "bg": "Four years after the shattered-glass reveal, the first Cybertrucks finally reached customers. On the October earnings call he had already confessed: “We dug our own grave with Cybertruck” by promising it so early — and warned of “enormous challenges” in volume production.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "e2024-01-29",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2024.01.29",
  "s": "X 帖（原帖 status/1752118131579867417 · The Guardian 报道）",
  "q": "“The first @Neuralink product is called Telepathy. Enables control of your phone or computer, and through them almost any device, just by thinking.”",
  "zh": "第一款 @Neuralink 产品名为 Telepathy（心灵感应）。只需思考，即可控制你的手机或电脑——并由此控制几乎所有设备。",
  "bg": "Eight years after founding Neuralink, the first human implant was done. He broke the news the way he breaks all news — on X, in two posts: one clinical, one naming the product and its promise.",
  "c": [
   "Neuralink"
  ]
 },
 {
  "id": "i2024-01-29",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2024.01.29",
  "s": "「Telepathy：只需思考，即可控制一切」",
  "q": "“The first @Neuralink product is called Telepathy. Enables control of your phone or computer, and through them almost any device, just by thinking.”",
  "zh": "",
  "c": [
   "Neuralink"
  ]
 },
 {
  "id": "p2024-01-29",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2024.01.29",
  "s": "@elonmusk",
  "q": "The first human received an implant from @neuralink yesterday and is recovering well. Initial results show promising neuron spike detection.",
  "zh": "首位人类昨日接受了 @neuralink 植入，恢复良好。初步结果显示神经元尖峰检测前景良好。",
  "c": [
   "X / Twitter",
   "Neuralink"
  ]
 },
 {
  "id": "p2024-03-11",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2024.03.11",
  "s": "@elonmusk",
  "q": "This week, @xAI will open source Grok",
  "zh": "本周，@xAI 将开源 Grok。",
  "c": [
   "X / Twitter",
   "xAI"
  ]
 },
 {
  "id": "e2024-06-13",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2024.06.13",
  "s": "Tesla 股东年会（CNBC · Teslarati 全文转录）",
  "q": "“I just want to start off by saying hot d***! I love you guys.” — later that day: humanoid robots “could lift Tesla's market cap to $25 trillion”.",
  "zh": "我只想先说一句：太爽了！我爱你们。——同日晚些时候：人形机器人「可能把 Tesla 的市值推到 25 万亿美元」。",
  "bg": "Delaware had voided his 2018 pay package; the same shareholders he once enraged were asked to re-ratify it, re-elect the board, and approve moving Tesla's incorporation to Texas. He flew in for the vote — and opened like a victory lap even before results were read.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "e2024-10-10",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2024.10.10",
  "s": "「We, Robot」发布会（What's Up Tesla 记录）",
  "q": "“Cybercab: The Autonomous Robotaxi. And then we've got the first car that is specifically built for unsupervised full self driving to be a robotaxi.”",
  "zh": "Cybercab：自主Robotaxi。这是我们第一辆专为无监督全自动驾驶而造的Robotaxi。",
  "bg": "Eight months after shelving the $25,000 car, the answer arrived on a movie-studio lot in Los Angeles: an event literally titled “We, Robot”, where the two-seat, no-pedal Cybercab was the star.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "e2024-10-13",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2024.10.13",
  "s": "X 帖（Reuters · AP 引述）",
  "q": "“The tower has caught the rocket!! … Big step towards making life multiplanetary was made today.” — “Science fiction without the fiction part.”",
  "zh": "塔接住了火箭！！……今天，向让生命多行星化迈出了一大步。——没有虚构部分的科幻。",
  "bg": "Starship's fifth test flight had regulatory clearance to attempt something never tried: bringing a 20-story-tall booster back not to a landing pad, but into the arms of the launch tower itself. Every prior flight had been about learning; this one was about catching.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "i2024-10-13",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2024.10.13",
  "s": "「塔接住了火箭」",
  "q": "“The tower has caught the rocket!! … Big step towards making life multiplanetary was made today.” — “Science fiction without the fiction part.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "p2024-10-13",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2024.10.13",
  "s": "@elonmusk",
  "q": "The tower has caught the rocket!! Big step towards making life multiplanetary was made today. Science fiction without the fiction part.",
  "zh": "塔接住了火箭！！……今天，向让生命多行星化迈出了一大步。——没有虚构部分的科幻。",
  "c": [
   "X / Twitter"
  ]
 },
 {
  "id": "e2025",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2025",
  "s": "Boring Company 官网项目页 · Nevada Current · Economic Times 报道",
  "q": "Vegas Loop — 68 miles · 104 stations · up to 90,000 passengers/hour.（Boring Company 官网项目页口径）",
  "zh": "Vegas Loop——68 英里、104 座车站、每小时最高 9 万人次。（公司官网项目页口径）",
  "bg": "The joke company quietly became an operator. Its Las Vegas Convention Center Loop (2.1 miles, 5 stations) carried real passengers for years, and the planned network grew to 68 miles and 104 stations — with Nashville and Dubai on the project list.",
  "c": [
   "Boring Company"
  ]
 },
 {
  "id": "e2025-05-03",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2025.05.03",
  "s": "Starbase 建市投票（Texas Tribune · TPR 报道）",
  "q": "“STARBASE IS AWESOME AND ANYONE CAN VISIT.”",
  "zh": "STARBASE 太棒了，任何人都可以来参观。",
  "bg": "SpaceX's South Texas launch site — where employees are the main residents — voted on whether to incorporate as a city. Of about 283 eligible voters, roughly 212 voted, overwhelmingly approving the Type C city of Starbase. His 2021 prediction had finally come true.",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "i2025-05-03",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2025.05.03",
  "s": "「STARBASE IS AWESOME」",
  "q": "“STARBASE IS AWESOME AND ANYONE CAN VISIT.”",
  "zh": "",
  "c": [
   "SpaceX"
  ]
 },
 {
  "id": "p2025-05-03",
  "pg": "x-posts.html",
  "t": "X 帖",
  "d": "2025.05.03",
  "s": "@elonmusk",
  "q": "STARBASE IS AWESOME AND ANYONE CAN VISIT.",
  "zh": "STARBASE 太棒了，任何人都可以来参观。",
  "c": [
   "SpaceX",
   "X / Twitter"
  ]
 },
 {
  "id": "e2025-09-01",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2025.09.01",
  "s": "Tesla 官方博客",
  "q": "“Sustainable abundance — by redefining the fundamental building blocks of labor, mobility and energy at scale and for all.”",
  "zh": "可持续富足——以规模化、面向所有人的方式，重新定义劳动、出行与能源这些基本构件。",
  "bg": "Twenty years after the secret plan, Tesla's product line was complete and its car business mature. The fourth master plan turned the page: from vehicles to AI, robots and autonomy.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "d2025-09-01",
  "pg": "documents.html",
  "t": "一手文档",
  "d": "2025.09.01",
  "s": "Master Plan Part IV",
  "q": "“Sustainable abundance — by redefining the fundamental building blocks of labor, mobility and energy at scale and for all.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "e2025-11-06",
  "pg": "primary.html",
  "t": "言行实录",
  "d": "2025.11.06",
  "s": "Tesla 股东年会（CNBC · Sky News · 全文转录）",
  "q": "“There's not like an obvious limit. So like Optimus is kind of like an infinite money glitch. And maybe there won't even be money in the future.” — “Optimus will actually eliminate poverty… bigger than cell phones.”",
  "zh": "这没有明显的上限。Optimus 有点像无限金钱外挂。也许未来甚至不会再有金钱这个东西。——Optimus 将真正消除贫困……比手机还要大（的市场）。",
  "bg": "The annual meeting where shareholders approved his trillion-dollar pay package (roughly 75% in favor) — and where he, fresh off the vote, took the stage and escalated the promise to its logical extreme.",
  "c": [
   "Tesla"
  ]
 },
 {
  "id": "i2025-11-06",
  "pg": "interviews.html",
  "t": "访谈与表态",
  "d": "2025.11.06",
  "s": "「无限金钱外挂」",
  "q": "“There's not like an obvious limit. So like Optimus is kind of like an infinite money glitch. And maybe there won't even be money in the future.” — “Optimus will actually eliminate poverty… bigger than cell phones.”",
  "zh": "",
  "c": [
   "综合"
  ]
 },
 {
  "id": "d2026-01",
  "pg": "documents.html",
  "t": "一手文档",
  "d": "2026.01",
  "s": "Series E（xAI 官方融资公告）",
  "q": "“We're building AI to accelerate human scientific discovery. Today, we're announcing that we have raised $20 billion at a $230 billion post-money valuation.”",
  "zh": "",
  "c": [
   "xAI"
  ]
 }
];
